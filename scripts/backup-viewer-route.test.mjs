...
describe('the route', () => {
  ...
  it('keeps the viewer as a top-level .html asset in dist, so /backup-viewer resolves natively', () => {
    // The artifact is generated by the build (public/backup-viewer.html is
    // git-ignored), so this is a dist assertion: skipped on a clean checkout,
    // mandatory under REQUIRE_DIST=1 (verify-viewer-dist.mjs, after the build).
    const viewerPath = fileURLToPath(new URL('../dist/backup-viewer.html', import.meta.url));
    if (!requireDist(viewerPath, expect)) return;
    expect(existsSync(viewerPath)).toBe(true);
    // At the TOP of dist — a nested copy would resolve to a different URL.
    expect(existsSync(fileURLToPath(new URL('../dist/backup-viewer/index.html', import.meta.url)))).toBe(false);
  });
