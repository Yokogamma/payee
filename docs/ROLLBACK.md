...
**Рекомендуется `--paid 4` × 6:** четыре запасные попытки, день запаса по
расписанию и один свободный слот под потолком. Больше даёт только подъём
`SOAK_MAX_PAID_TOTAL`, а это прямое увеличение расхода AR.
...
**Цель планирования** — 20 подтверждённых УСПЕХОВ, и это сознательно более
строгая величина: каждый успех даёт ровно один `upload_outcome`, поэтому 20
успехов гарантируют критерий, а 20 «исходов» — нет.

Отсюда ловушка, которую нельзя читать наоборот: окно, добравшее объём за счёт
`arweave_throw`, критерий объёма выполнит и **провалит** критерий строгого
нуля. Рост числа в `upload_outcomes` сам по себе не является хорошей новостью.

Строка `paid outcomes IN THE WINDOW` в `soak-d2.mjs status` считает
подтверждённые успехи драйвера — это ПЛАН, а не вердикт. Вердикт даёт
Analytics Engine, и там знаменатель другой.
