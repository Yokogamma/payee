...
Статус: **v15 — доработка протоколов по ревью 12** (2 high + 2 medium:
...
- **D10. Глобальный предохранитель горячего кошелька — предусловие
  ...
  **поштучные**: `RATE_LIMITER` берётся как
  `idFromName(publicKeyB64)` (`index.ts:640`), 20 публикаций/час на ключ
  ...
  транзакции**, (г) **метрика/аларм скорости сжигания**. **Денежный и
...
- **Ключ резервирования — `{publicKeyB64, noteId, gen}` (поправка v15,
  ...
  **Почему tenant обязателен.** Сегодня одинаковые `noteId` у разных
  ...
  `idFromName(publicKeyB64)` (`index.ts:640`); глобальная уникальность
  `noteId` НЕ обеспечивается — Worker проверяет лишь формат UUID и класс
  версии (`index.ts:604–612`), так что аллоулистнутый клиент волен прислать
  ...
  (Ed25519 над SHA-256 тела уже проверен, `index.ts:978`), никогда «как
  прислали». В метриках сырой ключ не публикуется (общий запрет §4.PR-2) —
  только производный бакет.
- **Повтор `reserve` с тем же ключом (ревью 12, gap):** совпадение tenant +
  ...
  всегда fail-closed.
- **`reserve(key, reward)` — ОДНА `storage.transaction`:** идемпотентно по
  ключу (повтор возвращает существующую резервацию, ничего не списывая);
  затем три проверки, все fail-closed:
...
- **Порядок в Worker:** per-key квота → котировка цены (transport adapter,
  PR-2 — `getPrice` уже вызывается ЯВНО) → `reserve` → подпись → durable
  `signed` → POST → `settle`. Тот же `reward`, что зарезервирован, передаётся
  в `createTransaction` — расхождение котировки и резервации запрещено.
- **`settle(key, outcome)`:** `accepted` → резервация переходит в
  `rollingSpend`; доказанное «не опубликовано» → освобождение; **неизвестный
  исход POST → резервация ОСТАЁТСЯ** и до выяснения трактуется как трата.
  Консервативно намеренно: ошибка в эту сторону = недорасход, ошибка в другую
  = пробитый floor.
- **Судьба «неизвестной» резервации (ревью 12, gap) — связана с
  reconciliation, а не с таймером.** Ротация окна её НЕ снимает: пока исход
  неизвестен, резервация висит как pending и продолжает уменьшать доступный
  остаток. Разрешение приходит от того же кворума статусов, что ведёт
  `signed`/`redrop_pending` (§ выше): вердикт `confirmed` → резервация
  списывается в `rollingSpend` ТЕКУЩЕГО окна; доказанный `dead` (+ age guard)
  → резервация освобождается в ТОЙ ЖЕ транзакции, где запись переходит в
  `redrop_pending` (на Arweave не попавшая в блок транзакция денег не стоит,
  поэтому освобождение здесь корректно, а новая generation получает СВОЮ
  резервацию). **TTL для таких резерваций намеренно НЕ вводится** — он
...
- **Доверие к балансу и цене — РАЗНЫЕ правила (переписано в v15, ревью 12
  ...
  - Сверх того котировка выше `MAX_TX_REWARD_WINSTON` → отказ (`503`), НЕ
    оплата; отклонение от согласованного значения дальше заданного
    множителя → отказ + метрика.
  - Короткий TTL-кэш для обоих значений; истёк и кворум не собрался → `503`.
...
- **D10 tenant и повтор (ревью 12):** ДВА пользователя с ОДИНАКОВЫМИ
  ...
  публикации не происходит; повтор `reserve` с тем же ключом и ДРУГИМ
...
- **D10 источники цифр (ревью 12):** один честный + один ЗАВЫШАЮЩИЙ баланс →
  ...
  ответа → медиана;
- **D10 неизвестный POST (ревью 12):** резервация переживает ротацию окна;
  последующий `confirmed` списывает её в текущее окно, доказанный `dead` +
  age guard освобождает её в той же транзакции, что открывает
  `redrop_pending`; удерживаемая сумма ограничена
...
- **D10 денежные грани (ревью 11):** транзакция, которая пробивает floor
  ...
  занижающий баланс одиночный шлюз НЕ останавливает сервис (медиана), как и
  завышающий не открывает слив;
...
- **Расхождение о НАЛИЧИИ txId (D11, v13, security-ревизия F3/R3) — отдельное
  ...
  факт «txId есть у источника A и отсутствует у B» ВСЕГДА даёт метрику
  ...
  объясняет. Так тихое умалчивание становится наблюдаемым, а рутинный лаг не
  превращает `incomplete` в постоянно взведённый флаг и не блокирует
  `sweep-full-at`.
- **Источник доказуемого возраста (v14, ревью 11 M1) — нормативно: кворум
  ...
  **Поправка v15 (ревью 12 M2): `block = null` ПОЛНОСТЬЮ исключён из решения
  ...
  - **правило согласия:** возраст установлен, когда **≥2 независимых
    status-origin** сообщают транзакцию подтверждённой и у обоих
    `number_of_confirmations ≥ MIN_PRESENCE_CONFIRMATIONS`. Расхождение
    `block_height` между origin в пределах нескольких блоков (разные вершины
    цепи) допустимо; расхождение о САМОМ факте включения или о блоке за
    пределами допуска ⇒ возраст НЕ установлен;
  ...
  может.
- **Бюджет проверок возраста (ревью 12, gap):** каждое расхождение стоит до
  пяти status-запросов, а расхождений в плохом сценарии много. Нормативно:
  результаты статусов кэшируются в пределах прогона; проверки идут в общей
  ограниченной параллельности и под ОБЩИМ дедлайном sweep-а; на прогон
  действует потолок числа проверяемых расхождений (приоритет — самые старые
  по локальным данным, затем произвольный детерминированный порядок).
...
## 8. Карта соответствия (статус v15)
...
| PR-3b write | спецификация завершена; **writer-релиз заблокирован предусловием D10 + ревью протокола** (reader — нет); ждёт своей очереди (§6 п.6) | **предохранитель кошелька в Winston: floor + бюджет + per-tx cap + атомарный tenant-scoped reserve/settle по `{publicKeyB64, noteId, gen}` + баланс-минимум / цена-медиана (security F5/R5, протокол v14–v15)**, согласованность alarm-теста/ADR/cap с durable-инвариантом (ревью 10), durable/postable-инвариант generation + `resign_violation` по durable-txId + `deadTxId`-lineage (ревью 9), durable-источник фазы 2 из сохранённого `signedTx` (ревью 8), durable `redrop_pending` + единый `recoveryCount`-cap (H1 ревью 7), `storage.transaction`-атомарность + try/finally + self-healing alarm (ревью 6), single-alarm scheduler + CAS + verdict-таблица (ревью 5), inline `signedTx` + cap (H2 ревью 4), `signed`/`redrop_pending` не подлежат release/TTL (ревью 3), reader-floor несёт scheduler + фазу 2 (H3/ревью 7), anchor expiry, staging paid-tx испытания → утверждение `UPLOAD_GATEWAYS` |
| PR-4 union | risky; **v15: union не удаляет txId (арбитр — header D9), возраст — только кворум статусов в подтверждениях; подлежит ревью** | **D11: presence-расхождение → метрика всегда, `incomplete` по порогу давности; restore остаётся аддитивным (security F3/R3)**, логические `INDEX_SOURCES` (M1 ревью 4), single-index edge-order сохранён (M2), metadata-конфликт→incomplete, nullable height, abort-backoff |
