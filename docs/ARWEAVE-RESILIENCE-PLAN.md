...
Статус: **v14 — протоколы к D9–D11 дописаны по ревью 11** (2 high + 1 medium
...
протоколов PR-3a, writer-часть PR-3b и PR-4 НЕ стартуют — PR-1/PR-2 не
заблокированы. Предыдущий шаг: **v13 — интегрирована security-ревизия контура**
...
- **Единицы — Winston** (1 AR = 10¹² Winston), НЕ «число публикаций»: лимит
  ...
  независимых предела: `WALLET_FLOOR_WINSTON` (неприкосновенный остаток),
...
- **Новый глобальный DO `SpendGuard`** (`idFromName('global')`) — отдельно от
  ...
  rollback floor.
- **Ключ резервирования — `{noteId, gen}`, а НЕ txId.** Он известен ДО
...
- **`settle(key, outcome)`:** `accepted` → резервация переходит в
  ...
  исход POST → резервация ОСТАЁТСЯ** и в дальнейшем трактуется как трата.
  Консервативно намеренно: ошибка в эту сторону = недорасход, ошибка в другую
  = пробитый floor.
- **Доверие к балансу и цене (недоверенный шлюз — часть модели):** баланс —
  `GET /wallet/<address>/balance`, цена — `/price/<bytes>` с настроенных
  шлюзов; берётся **медиана по ≥2 ответившим** источникам. Медиана, а не
  минимум и не максимум: минимум позволил бы ОДНОМУ занижающему шлюзу
  остановить весь сервис, максимум — одному завышающему открыть слив.
  Короткий TTL-кэш; протухло и свежей медианы нет → fail-closed `503`.
  Котировка выше `MAX_TX_REWARD_WINSTON` или уходящая от медианы дальше
  заданного множителя → отказ (`503`), НЕ оплата. (Живая проба 2026-08-20:
  `/price/1024` совпал до цифры на arweave.net, vilenarios.com и ar-io.dev —
  расхождение медианы будет заметным сигналом, а не шумом.)
...
- **Расхождение о НАЛИЧИИ txId (D11, v13, security-ревизия F3/R3) — отдельное
  ...
  txId **доказуемо** старше порога давности, при котором отставание индекса
  его уже не объясняет. Порог калибруется в этом PR по данным PR-2 (ориентир
  — `MIN_COMMITTED_AGE_MS`). Так тихое умалчивание становится наблюдаемым, а
  рутинный лаг не превращает `incomplete` в постоянно взведённый флаг и не
  блокирует `sweep-full-at`.
- **Источник доказуемого возраста (v14, ревью 11 M1) — нормативно: кворум
  ...
  поэтому она НИКОГДА не управляет флагом (только упорядочиванием кандидатов,
  §PR-4 выше). Следствия, все нормативные:
  - `block = null` у индекса (не смайнено / не проиндексировано) ⇒ по
    определению «свежая» ⇒ `incomplete` НЕ поднимается никогда;
  - **чистое устройство** локального `firstSeenAt` не имеет — правило
    работает и там, потому что не зависит от локального состояния;
  - на устройствах, где локальный `firstSeenAt` есть, он может служить
    ДОПОЛНИТЕЛЬНОЙ нижней границей («видели давно — значит старая»), но
    никогда единственным источником;
  - кворум статусов не смог установить возраст (источники недоступны или
    разошлись) ⇒ метрика есть, флага НЕТ. Недоказуемый случай не имеет права
    поднимать пользовательский сигнал.
  ...
  status-источников), а скрыть расхождение тоже не может (union его уже
  увидел и метрика взведена).
...
**`block = null` → флага нет никогда; чистое устройство без `firstSeenAt` →
правило работает от кворума статусов; кворум не установил возраст → метрика
есть, флага нет; индекс сообщает заведомо старую высоту, а статусы говорят
иное → выигрывают статусы** (индекс не управляет флагом); оба индекса опускают →
...
1. PR-1: ADR + этот план v14 (D7 утверждён — §2.1; формула кворума и
...
## 8. Карта соответствия (статус v14)
