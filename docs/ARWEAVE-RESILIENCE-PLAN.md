...
Статус: **v13 — интегрирована security-ревизия контура**
...
- **D9. Верификация `txId ↔ bytes` — ОБЯЗАТЕЛЬНОЕ предусловие
  ...
  arweave-js в бандле. Цепочка привязки: `id === SHA-256(signature)` →
  RSA-PSS-проверка подписи над deep-hash полей (включая `data_root`, `owner`,
  теги) → сверка `data_root` с полученными байтами. Точная формула листа —
  по `merkle.ts` из arweave-js, закрепляется тестом против реальной
  транзакции. Побочный эффект: владелец транзакции становится
  АУТЕНТИФИЦИРОВАННЫМ, что и закрывает F1 по существу (см. §1).
...
- **D10. Глобальный предохранитель горячего кошелька — предусловие
  ...
  **поштучные**: `RATE_LIMITER` берётся как
  ...
  Worker появляются (а) **порог баланса** — отказ публиковать
  (fail-closed `503`) ниже floor; (б) **общий скользящий бюджет публикаций**
  отдельным глобальным DO (не per-key); (в) **метрика/аларм скорости
  сжигания**. Проверка — непосредственно перед платной публикацией
  (`index.ts:743`), после всех per-key/per-IP/идемпотентных проверок.
  Осознанная цена: сработавший предохранитель деградирует ВСЕХ до `503` —
  это верный fail-closed выбор для bearer-актива, но он обязан идти в паре
  с оповещением оператора.
...
- **Гейт D9 (v13, security-ревизия F2/R2) — нормативно:** пул из >1
  payload-шлюза НЕ ВЫПУСКАЕТСЯ без проверки соответствия байтов запрошенному
  txId. Проверка идёт ПЕРЕД расшифровкой; провал — это отказ ШЛЮЗА (метрика
  `payload_hash_mismatch` + следующий шлюз), а НЕ «кандидат не
  расшифровался», иначе подмена неотличима от штатного пропуска мусора.
  Реализация без arweave-js: единственный чанк по построению
  (`MAX_BODY_BYTES = 51200` ≪ 256 KiB), привязка
  `id === SHA-256(signature)` → RSA-PSS над deep-hash полей → `data_root` от
  байтов. Замер бандла обязателен (запас 2.7 KB gz): не влезло — выпускается
  одиночный `arweave.net`, а не пул без проверки.
- **Побочно закрывается F1:** владелец транзакции становится
  аутентифицированным, а не «полем, которое вернул шлюз». В этом же PR
  правится вводящий в заблуждение комментарий `arweave.ts:484–486` (§1).
...
НЕ выпускается, пока в Worker нет: (а) порога баланса кошелька (ниже floor —
`503`, публикации нет); (б) общего скользящего бюджета публикаций в отдельном
глобальном DO; (в) метрики и аларма скорости сжигания. Проверка — прямо перед
платной публикацией (`index.ts:743`). Reader-релиз этим не блокируется: он не
создаёт `signed` и не открывает фазу 2.
...
- **D10 (security):** баланс ниже floor → публикации нет (`503`) ДО подписания;
  превышен общий бюджет публикаций → `503` независимо от того, что per-key
  квота свободна; K ключей на своих лимитах упираются в общий потолок;
...
- **Расхождение о НАЛИЧИИ txId (D11, v13, security-ревизия F3/R3) — отдельное
  ...
  txId старше порога давности, при котором отставание индекса его уже не
  объясняет. Порог калибруется в этом PR по данным PR-2 (ориентир —
...
взведена; тот же txId СТАРШЕ порога давности → дополнительно `incomplete=true`;
СВЕЖИЙ txId (лаг) → `incomplete` НЕ поднимается; оба индекса опускают →
...
1. PR-1: ADR + этот план v13 (D7 утверждён — §2.1; формула кворума и
...
## 8. Карта соответствия (статус v13)
...
| PR-3a read | ready; **объём вырос на гейт D9** | **верификация `txId ↔ bytes` перед пулом payload + правка комментария F1 (security)**, строгий N-of-N (H1 ревью 4), `MIN_STATUS_ORIGINS=2`, dedup по типу (H5), payload-validation fallback, acceptance «редирект не ломает /raw» |
| PR-3b write | спецификация завершена; **writer-релиз заблокирован предусловием D10** (reader — нет); ждёт своей очереди (§6 п.6) | **глобальный предохранитель кошелька: floor + общий бюджет + аларм сжигания (security F5/R5)**, согласованность alarm-теста/ADR/cap с durable-инвариантом (ревью 10), durable/postable-инвариант generation + `resign_violation` по durable-txId + `deadTxId`-lineage (ревью 9), durable-источник фазы 2 из сохранённого `signedTx` (ревью 8), durable `redrop_pending` + единый `recoveryCount`-cap (H1 ревью 7), `storage.transaction`-атомарность + try/finally + self-healing alarm (ревью 6), single-alarm scheduler + CAS + verdict-таблица (ревью 5), inline `signedTx` + cap (H2 ревью 4), `signed`/`redrop_pending` не подлежат release/TTL (ревью 3), reader-floor несёт scheduler + фазу 2 (H3/ревью 7), anchor expiry, staging paid-tx испытания → утверждение `UPLOAD_GATEWAYS` |
| PR-4 union | risky | **D11: presence-расхождение → метрика всегда, `incomplete` по порогу давности; restore остаётся аддитивным (security F3/R3)**, логические `INDEX_SOURCES` (M1 ревью 4), single-index edge-order сохранён (M2), metadata-конфликт→incomplete, nullable height, abort-backoff |
