...
| `TRUSTED_OWNERS` | **non-secret config** in `wrangler.toml`, declared in EVERY named environment | worker `/upload` (D2/D9) | not a secret — these addresses are on chain and compiled into the client bundle. What matters is that the list only ever GROWS: the worker authenticates a publication before binding its `txId` to a payload fingerprint, and a dropped address does not fail loudly — it makes the project's own older publications unverifiable, so healthy records start answering a conflict or a 503. Deliberately NOT derived from `ARWEAVE_JWK`: after a rotation the current wallet cannot confirm older transactions. What it must contain is pinned in `scripts/owner-pins.mjs` and enforced by `check-trusted-owners.mjs`, which also requires the worker and client sets to agree — per contour: production EQUALS the client; staging equals it plus EVERY wallet pinned in `STAGING_ONLY_OWNERS` (`scripts/owner-pins.mjs`, append-only with the floor `STAGING_NEVER_REMOVE`), none of which may ever appear in the registry, production or the client. Missing, empty or malformed → `/upload` 503s (fail-closed), and so does a signing wallet outside the set. **Rotation order is asymmetric, and there are two rotations:** the project (dev) wallet — add the new address to the registry, both worker tables and the client, deploy FIRST, switch `ARWEAVE_JWK` second; staging's OWN wallet — APPEND it to `STAGING_ONLY_OWNERS` + `STAGING_NEVER_REMOVE` and to `env.staging.TRUSTED_OWNERS` only (never production or the client), deploy staging FIRST, switch staging's `ARWEAVE_JWK` second; the old staging address stays |
