...
- после листинга каждый срез **пересчитывается самим хранилищем** (представление
  ...
  столько строк, сколько хранилище насчитало, — меньше или больше значит, что
  ЭТОТ инструмент читает не всё, и выгрузка обрывается. И только этот путь
...
- **не дефект экспортёра и не задержка.** Обе инвокации отсутствуют во всех
  доступных представлениях API: в листинге событий с фильтром по воркеру и
  без фильтра, по точечному фильтру `$metadata.rayId = <ray>` (соседняя
  инвокация тем же фильтром находится), по фильтру на nonce, в собственном
  пересчёте хранилища (`calculations`: 263 уникальных `rayId` за час — ровно
  столько же, сколько в листинге) и в представлении `invocations`. Спустя
  33 часа — та же картина. Эти два события в хранилище не читаются ничем;
...
- **обрывает выгрузку**, если листинг вернул не столько строк, сколько
  хранилище насчитало, — это дефект чтения, и молча короткий архив хуже
  отсутствующего;
