...
0. **Получить доступ к Workers Observability ЗАРАНЕЕ** — токен со scope
   ...
   - **exit 0, ноль событий** — доступ подтверждён. Ноль здесь ожидаем: сбор
     ещё не включён, и это нормально;
   - **`HTTP 401` / `HTTP 403` / `API reported failure`** — токен отозван,
     не тот или без scope. Чинить ДО деплоя, иначе окно хранения будет
     тратиться на выяснение прав;
   - **`none of the known script-name keys`** — авторизация прошла, но у
     хранилища ещё нет схемы, потому что ничего не собиралось. Это НЕ проблема
     доступа и до включения сбора ожидаемо; перепроверить после деплоя.
