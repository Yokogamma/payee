...
| `money_reconcile` | event, outcome (`spent` / `released` / `wait` / `watch`), result (`settled` / `noop` / `unknown` / `terminal_refusal` / `retry` / `pending` / `expired` / `abandoned`) | attempts so far | the per-key money index (review 24.09 #2 H4, #4 H2): one step of settling a POSTed txId's reservation by the quorum, independent of client rechecks. `released` never ends the entry: it becomes a `watch` (the txId had a permit, so a late landing is still possible) until the anchor bound has passed AND the pool still says dead (`watch`/`expired`); a watch with no dead verdict inside `MONEY_WATCH_MAX_MS` is dropped as `abandoned` (with a log line). A late money quorum on a watched txId is booked `released → spent` by this step (`spent`/`settled`, the lattice's `spend_conflict`) |
