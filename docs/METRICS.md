...
**С 2026-09-13 (PR #166/#167) источник вердикта — журнал операций** («The
operation journal» выше) и ledger драйвера, сведённые `soak-d2.mjs reconcile`;
таблица ниже описывает, что даёт КАЖДЫЙ канал, и почему телеметрия осталась
диагностикой. Критерии v2 — docs/ROLLBACK.md «Soak criteria — v2».

| Критерий (docs/ROLLBACK.md «Exit», v2) | Источник вердикта | Диагностика (не вердикт) |
|---|---|---|
| `conflict`, `redrop_conflict`, `legacy_not_ours`, `recovery_conflict` — **строго 0** | журнал: `finished` с таким исходом или attest; незавершённая `begun` с `checkVerdict = id_payload_conflict` (вердикт DO пережил потерю ответа) | `semantic_idempotency` в AE (свой индекс на исход), строка `{"critical":…}` в логах, `STOP` драйвера |
| `arweave_throw` — **строго 0** (в узком смысле: throw в собственной подготовке) | журнал: исход `arweave_throw`, `paidResult: none` | `upload_outcome` в AE, `{"critical":"arweave_throw"}` в логах |
| `gateway_unavailable_pre_post`, `audit_aborted` — допуск владельца | журнал: исходы; инциденты по уникальным операциям | `gateway_call` в AE (класс `network`/`timeout`) |
| `post_unknown` — красное до `resolve` | журнал: исход `post_unknown` или запись `posting`, подписанный txId сохранён ДО POST | `post_accepted` против `upload_outcome` в AE — оценка, не доказательство |
| нет неучтённых результатов | сверка двух книг по `operationId`: `answer_without_record`, `unresolved_no_record`, `foreign_operation`, `mismatch`, `unfinished_*`, `unresolved_paid` — все красные | — |
| `legacy_unproven`, `recovery_unproven` — ≤ 1, 0 в последние 48 ч | журнал: attest/исход с временем | AE, логи; `unprovenSeen` в ledger (только legacy) |
| доля успеха ≥ 95 % | журнал: `accepted` (+ разрешённые вручную) ÷ `UPLOAD_OUTCOMES` — тот же знаменатель, что был у `upload_outcome` | `upload_outcome` в AE — порядок величины |
| объём (30 решений, 3 дня, 10 дедупов, 3 бэкфилла по разным заметкам, 20 платных) | журнал: `finished`-записи окна и их `attests` | `summarize` драйвера — план; `semantic_idempotency` в AE — оценка |

**Что из этого следует.** Ноль по любому критерию теперь означает «в журнале
нет ни записи с таким исходом, ни незавершённой записи, ни неучтённой отправки
драйвера» — при явных предпосылках, что журнал хранится и инструментирование
корректно (тесты 1–19 плана). Ограничение `recovery_*` остаётся: путь
недостижим для драйвера, ноль там — «не наблюдалось», а не «проверено
сценарием». И журнал не доказывает, что запрос, не дошедший до воркера, не был
отправлен: такая неизвестность красная, пока нет протокола закрытия допуска.
...
### Диагностика отказов `/admin/metrics`

При отказе чтения Analytics Engine обработчик пишет через `logStructured`
строку с `diagnostic: "metrics_upstream_failure"`. Поля: `stage`, `report`,
`hours`, доступный `upstreamStatus`, структурно проверенные `requestRay` и
`upstreamRay`, а также `errorCodes` — до восьми уникальных неотрицательных
целых кодов из `errors[].code`. Отсутствующие или некорректные CF-Ray опускаются.
Стадии: `fetch_failed`, `http_status`, `body_read_failed`, `body_too_large`,
`invalid_json`, `invalid_shape`. Статус и CF-Ray при HTTP-отказе сохраняются
даже при невозможности прочитать тело. Чтение ограничено 256 KiB и сигналом
таймаута запроса в 10 секунд.

Сообщения upstream, тело, SQL, URL и секреты не логируются. Ответ клиенту
остаётся общим; успешный запрос этой строки не создаёт. Это диагностическая
строка, не критический исход соака. Отсутствие строки не доказывает отсутствие
ошибки, а код аутентификации сам по себе не устанавливает факт отзыва токена.
После одновременного исправления секрета и кода исторический отказ таким
способом восстановить нельзя.

Патч применяется только после фактического закрытия и архивирования текущего
окна соака. Это касается и смены секрета: обычный `wrangler secret put` сразу
активирует новую версию. Совместное применение кода и секрета одной активацией
нужно отдельно подготовить через доверенный workflow.
...
данной проверке**, и ровно столько она устанавливает. Причина НЕ установлена:
наблюдение не исключает ни более долгую задержку, ни особенности выборки, ни
оставшийся дефект экспортёра; `abr_level = 1` и `head_sampling_rate = 1` —
свойства этих ответов и этой конфигурации, причину они не называют. Частота
потерь канала из двух всплесков не выводится. Теряются ли структурные строки
внутри доставленной инвокации независимо от неё — не измерено (двух
контрольных событий для этого мало). Практическое следствие для критериев не
зависит от причины: **ноль `critical`-строк в логах — не доказательство нуля
исходов**, а второй канал остаётся тем, чем объявлен, — способом увидеть
исход, который Analytics Engine мог отсэмплировать, без гарантии увидеть
каждый.
