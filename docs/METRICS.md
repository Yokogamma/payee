...
Выполняется ПОСЛЕ отдельно разрешённого деплоя в dev и ДО открытия окна.
...
6. **Отказ от перезаписи.** Повторить шаг 2 с теми же `--from/--to`: команда
   обязана отказать («archive already exists»). С относительным `--hours`
   проверка не работает — интервал каждый раз новый.
